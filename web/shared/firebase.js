...
import { addDoc, collection, deleteDoc, doc, getDoc, getDocs, getFirestore, orderBy, query, Timestamp, where } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js";
...
export async function savePlanToFirestore(name, days, exercises) {
    try {
        ...
        for (const ex of exercises) {
            ...
            const [min, max] = ex.repsRange.split('-').map(s => parseInt(s.trim(), 10));
...
export async function saveWorkoutSession(planId, exercises) {
    try {
        const user = getAuth().currentUser;
...
export async function saveMealToFirestore(name, calories) {
    try {
        const user = getAuth().currentUser;
        if (!user) throw new Error("Użytkownik niezalogowany!");
...
export async function saveNotification(title, body, hour, minute) {
    try {
        const user = getAuth().currentUser;
        if (!user) throw new Error("Użytkownik niezalogowany!");
...
export async function deleteUserNotification(userId) {
    try {
